...
class RailwayTrack:
    ...
    def __init__(self, length, deflection, head_station="", tail_station="",
                 ...
                 up_direction="N", down_direction="S", **kwargs):
        self.length = length
...
def load_train_graph_from_db(db, graph_name=None):
    ...
    try:
        g = conn.execute(
            'SELECT name, length, width, scale, default_scale, default_speed FROM train_graph LIMIT 1').fetchone()
        if not g:
        ...
        train_graph = TrainGraph(name=g[0], length=g[1], width=g[2], scale=g[3],
                                 ...
                                 default_speed=g[5] if len(g) > 5 and g[5] is not None else 1.0)
        ...
        for prow in paths:
            ...
            for hs, ts, length, deflection, dh, dt, lf, ud, dd in tracks:
                path.add_track(RailwayTrack(
...
def save_train_graph_to_db(train_graph, db):
    ...
    try:
        ...
        conn.execute(
            'INSERT OR REPLACE INTO train_graph (name, length, width, scale, default_scale, default_speed) '
            'VALUES (?,?,?,?,?,?)',
